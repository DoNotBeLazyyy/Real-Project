...
export async function changePassword(req, res) {
    ...
    try {
        ...
        const [result] = await pool.query(sql, [hashed, username]);
        const [rows] = await pool.query('SELECT username, password, initialPassword, userRole FROM account WHERE username = ?', [username]);
        if (!rows.length) {
